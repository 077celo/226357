...
class UpgradeNodes(bpy.types.Operator):
    ...
    def execute(self, context):
        context.scene.render_raw_scene.is_upgrading = True
        context.scene.render_raw_scene.enable_RR = True
        context.scene.render_raw_scene.is_upgrading = False
        # Setting the old enable property to False to keep things clean
...
class RefreshNodeTree(bpy.types.Operator):
    ...
    def execute(self, context):
        refresh_RR_nodes(context, include_current=True)
        return{'FINISHED'}
...
class UnlinkActiveGroup(bpy.types.Operator):
    ...
    def execute(self, context):
        RR = Settings(context, use_cache=False)
