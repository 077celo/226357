...
from ..utilities.view_transforms import get_view_transforms, update_view_transform, update_display
...
from ..nodes.update_colors import (
    ...
    update_value_saturation, update_color_boost, update_color_panel, update_preserve_color,
    update_density, update_split_tone
)
from ..nodes.update_details import (
    update_sharpness, update_texture, update_clarity, update_negative_bleed, update_details_panel
)
from ..nodes.update_effects import (
    update_vignette, update_glare, update_bloom, update_streaks, update_ghosting, update_halation,
    update_distortion, update_grain, update_effects_panel
)
from ..nodes.update_values import (
    ...
    update_pre_curves, update_post_curves, update_log_curves, update_fix_clipping
)
from ..nodes.update_alpha import update_alpha
from ..nodes.update_layer import update_active_layer, update_layer_name, update_layer_factor
...
class RenderRawSettings(bpy.types.PropertyGroup):
    ...
    view_transform: bpy.props.EnumProperty(
        name = 'View Transform',
        description = 'Transform used when converting an image to a display space',
        items = lambda self, context: get_view_transforms(context),
        default = None, # Must be None for items to be dynamic
        update = lambda self, context: update_view_transform(context, RR_group=self.id_data)
    )
    display: bpy.props.EnumProperty(
        name = 'Display Target',
        description = 'The intended display space of the view and render',
        items = [
            ('sRGB', 'sRGB', ''),
            ('Display P3', 'Display P3', ''),
            ('Rec.1886', 'Rec.1886', ''),
            ('Rec.2020', 'Rec.2020', ''),
            ('Rec.2100-PQ', 'Rec.2100-PQ', ''),
            ('Rec.2100-HLG', 'Rec.2100-HLG', ''),
        ],
        default = 'sRGB',
        update = lambda self, context: update_display(context, RR_group=self.id_data)
    )
    ...
    alpha_method: bpy.props.EnumProperty(
        ...
        default = '1',
        update = lambda self, context: update_alpha(context, RR_group=self.id_data)
    )
    ...
    display_curves: bpy.props.EnumProperty(
        name = 'Curves',
        items = [
            ('PRE', 'Pre', 'Displays linear pre-transform curves. They will not cause clipping but are harder to use because they are not bound to the 0-1 range'),
            ('LOG', 'Log', 'Displays logarithmic space curves. They will not cause clipping and are restricted to the zero to one range, though usually values are squished close to middle grey'),
            ('POST', 'Post', 'Displays post-transform sRGB curves. They can easily break the image but are easier to use because all values are within the 0-1 range'),
        ],
        default = 'POST'
    )
    ...
    layer_factor: bpy.props.FloatProperty(
        ...
        min = 0,
        max = 1,
        update = lambda self, context: update_layer_factor(self, context)
    )
    ...
    contrast_shadows: bpy.props.FloatProperty(
        name = 'Shadows Contrast',
        description = "Adjusts the exposure and gamma at the same time pre-transform to increase contrast only in the darker areas",
        default = 0,
        min = -1,
        max = 1,
        update = lambda self, context: update_contrast(self, context)
    )
    contrast_highlights: bpy.props.FloatProperty(
        name = 'Highlights Contrast',
        description = "Adjusts the exposure and gamma at the same time pre-transform to increase contrast only in the lighter areas",
        default = 0,
        min = -1,
        max = 1,
        update = lambda self, context: update_contrast(self, context)
    ...
    # Log curves
    log_curves_factor: bpy.props.FloatProperty(
        name = 'Curves Factor',
        description = 'Adjusts how much the RGB curves get applied',
        default = 1,
        min = 0,
        max = 1,
        update = lambda self, context: update_log_curves(self, context)
    )
    log_curves_black: bpy.props.FloatVectorProperty(
        name = 'Black Level',
        description = "Color considered as black by the pre-transform curve",
        subtype = 'COLOR_GAMMA',
        default = [0, 0, 0],
    ...
    log_curves_white: bpy.props.FloatVectorProperty(
        name = 'White Level',
        description = "Color considered as white by the pre-transform curve",
        subtype = 'COLOR_GAMMA',
        default = [1, 1, 1],
    ...
    log_curves_tone: bpy.props.EnumProperty(
        name = 'Tone',
        items = [
            ('STANDARD', 'RGB', 'Curve is applied to each channel individually, which may result in a change of hue'),
            ('FILMLIKE', 'Filmlike', 'Keeps the hue constant while removing control over individual channels'),
    ...
    split_tone: bpy.props.FloatProperty(
        name = 'Split Tone',
        description = 'A positive value warms the highlights and cools the shadows while a negative value does the opposite, post-transform',
        default = 0,
        min = -1,
        max = 1,
        update = lambda self, context: update_split_tone(self, context)
    )
    ...
    saturation_perceptual: bpy.props.FloatProperty(
        name = 'Perceptual',
        description = (
            'A value of 1 keeps the perceived value the same during saturation adjustments rather than the RGB value, which is more intuitive but can cause colors to clip. \n\n' \
            'A value of zero darkens the saturated areas to try to keep them in range, which is known as subtractive saturation. It is better for increasing saturation but worse for lowering saturation. \n\n' \
            'The default of 0.5 attempts to be smart and find the best approach per pixel'
            ),
        default = 0.5,
        min = 0,
        max = 1,
    ...
    color_boost_perceptual: bpy.props.FloatProperty(
        name = 'Perceptual',
        description = (
            'A value of 1 keeps the perceived value the same during saturation adjustments, while a value of zero darkens the saturated areas'
            ),
        default = 1,
        min = 0,
        max = 1,
    ...
    density: bpy.props.FloatProperty(
        name = 'Density',
        description = 'Saturates already saturated areas even more and reduces the brightness to avoid clipping',
        default = 0,
        min = 0,
        max = 1,
        update = lambda self, context: update_density(self, context)
    )
    density_depth: bpy.props.FloatProperty(
        name = 'Density Value Mask',
        description = 'Focuses the density control on bright areas when positive or dark areas when negative',
        default = 0,
        min = -1,
        max = 1,
        update = lambda self, context: update_density(self, context)
    ...
    hue_perceptual: bpy.props.FloatProperty(
        ...
        description = (
            ...
            'but can cause clipping when increasing saturation post-transform in bright areas'
        ),
        default = 1,
        min = 0,
        max = 1,
    ...
    hue_linear: bpy.props.FloatProperty(
        name = 'Linear Blend',
        description = 'Adjusts the hue pre-transform, in linear space, rather than post-transform',
        default = 0,
        min = 0,
        max = 1,
        update = lambda self, context: update_hue(self, context)
    ...
    lgg_factor: bpy.props.FloatProperty(
        name = 'Factor',
        description = "Scales the Lift Gamma Gain controls which is useful for making them less touchy",
        default = 0.5,
        min = 0,
        max = 1,
        update = lambda self, context: update_color_balance(self, context)
    )
    ops_factor: bpy.props.FloatProperty(
        name = 'Factor',
        description = "Scales the Offset Power Slope controls which is useful for making them less touchy",
        default = 0.5,
        min = 0,
        max = 1,
        update = lambda self, context: update_color_balance(self, context)
    ...
    if bpy.app.version >= (5, 0, 0):
        # Previous versions needed the subtype to be COLOR_GAMMA
        # while newer versions need it to be COLOR
    ...
    halation: bpy.props.FloatProperty(
        name = 'Halation',
        description = "The small warm glow along high contrast edges that can occur when shooting on real film",
        default = 0.5,
        min = 0,
        max = 1,
        update = lambda self, context: update_halation(self, context)
    )
    halation_size: bpy.props.FloatProperty(
        name = 'Halation Size',
        description = "The width of the halation effect",
        default = 1,
        min = 0,
        max = 1,
        update = lambda self, context: update_halation(self, context)
    ...
    horizontal_dispersion: bpy.props.BoolProperty(
        name = 'Horizontal',
        description = 'Uses the horizontal dispersion method rather than the radial one',
        default = False,
        update = lambda self, context: update_distortion(self, context)
    )
    ...
    grain_method: bpy.props.EnumProperty(
        ...
        items = [
            ...
            ('ACCURATE', 'Accurate', 'Simulates grain realistically by displacing the image'),
        ],
        default = 'FAST',
        update = lambda self, context: update_grain(self, context)
    )
    ...
    negative_bleed: bpy.props.FloatProperty(
        name = 'Strength',
        description = "Simulates light bleed during film negative exposure, leading to darkening near shadows",
        default = 0,
        min = 0,
        max = 1,
        update = lambda self, context: update_negative_bleed(self, context)
    )
    negative_bleed_size: bpy.props.FloatProperty(
        name = 'Size',
        description = "How big of an area the bleed spreads over",
        default = 0,
        min = 0,
        max = 1,
        update = lambda self, context: update_negative_bleed(self, context)
...
def register():
    ...
    # This property group is only saved to the scene to correctly read from versions before 1.2
