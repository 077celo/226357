...
def reset_layer_settings(context, RR_group, active_layer_index=None):
    ...
    nodes_pre = layer_pre.node_tree.nodes

    if layer_pre.node_tree.render_raw.layer_factor != 1:
        layer_pre.node_tree.render_raw.layer_factor = 1
    ...
    set_curve_node(nodes_pre['Curves'], RGB_curve_default)
    set_curve_node(nodes_pre['Log Curves'], RGB_curve_default)
    ...
    update_all(None, context, RR_group, active_layer_index)
...
def add_layer(context, RR_group, name=None):
    ...
    shift_index()
    reset_layer_settings(context, RR_group)
