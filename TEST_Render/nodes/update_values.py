...
def update_exposure(self, context, RR_group=None, layer_index=None):
    ...
    if (
        ...
        EXPOSURE.mute = False
        EXPOSURE.inputs['Exposure'].default_value = RR.props_pre.exposure


def update_group_exposure(context, RR_group):
    RR = Settings(context)
...
def update_gamma(self, context, RR_group=None, layer_index=None):
    ...
    if (
        ...
        GAMMA.mute = False
        GAMMA.inputs['Gamma'].default_value = RR.props_pre.gamma


def update_group_gamma(context, RR_group):
    RR = Settings(context)
    GAMMA = RR_group.nodes['Gamma']

    if RR.props_scene.gamma == 1:
        GAMMA.mute = True
    else:
        GAMMA.mute = False
...
def update_contrast(self, context, RR_group=None, layer_index=None):
    ...
    if (
        ...
        not is_layer_used(RR) or (
            RR.props_pre.contrast == 0 and
            RR.props_pre.contrast_shadows == 0 and
            RR.props_pre.contrast_highlights == 0
        )
    ):
        CONTRAST.mute = True
    else:
        CONTRAST.mute = False
        CONTRAST.inputs['Total'].default_value = RR.props_pre.contrast
        CONTRAST.inputs['Shadows'].default_value = RR.props_pre.contrast_shadows
        CONTRAST.inputs['Highlights'].default_value = RR.props_pre.contrast_highlights
...
def update_pre_curves(self, context, RR_group=None, layer_index=None):
    ...
    CURVES.inputs['Fac'].default_value = RR.props_pre.pre_curves_factor
...
def update_log_curves(self, context, RR_group=None, layer_index=None):
    RR = Settings(context, RR_group, layer_index)
    CURVES = RR.nodes_pre['Log Curves']
    CURVES.mute = not RR.use_values or not is_layer_used(RR)
    CURVES.inputs['Fac'].default_value = RR.props_pre.log_curves_factor
    CURVES.mapping.tone = RR.props_pre.log_curves_tone
    CURVES.inputs['Black Level'].default_value = set_alpha(RR.props_pre.log_curves_black, 1)
    CURVES.inputs['White Level'].default_value = set_alpha(RR.props_pre.log_curves_white, 1)
...
def update_post_curves(self, context, RR_group=None, layer_index=None):
    ...
    CURVES.mute = not RR.use_values or not is_layer_used(RR)
    CURVES.inputs['Fac'].default_value = RR.props_post.post_curves_factor
...
def update_fix_clipping(self, context, RR_group=None, layer_index=None):
    ...
    if not is_layer_used(RR) or not RR.use_values or PROPS.fix_clipping == 0:
        FC.mute = True
    else:
        FC.mute = False
        FC.inputs['Strength'].default_value = PROPS.fix_clipping
...
def update_value_panel(self, context, RR_group=None, layer_index=None):
    ...
    update_fix_clipping(self, context, RR_group, layer_index)
