...
def update_preserve_color(self, context, RR_group=None, layer_index=None):
    ...
    if not is_layer_used(RR) or not RR.use_colors or is_default:
        ...
        PH.mute = False
        PH.inputs['Filmic'].default_value = PROPS.preserve_filmic
        PH.inputs['Hue'].default_value = PROPS.preserve_hue
        PH.inputs['Saturation'].default_value = PROPS.preserve_saturation
...
def update_saturation(self, context, RR_group=None, layer_index=None):
    ...
    if not is_layer_used(RR) or not RR.use_colors or PROPS.saturation == 1:
        ...
        SATURATION.inputs['Saturation'].default_value = PROPS.saturation
...
def update_color_boost(self, context, RR_group=None, layer_index=None):
    ...
    if not is_layer_used(RR) or not RR.use_colors or PROPS.color_boost == 0:
        ...
        BOOST.mute = False
        BOOST.inputs['Strength'].default_value = PROPS.color_boost
        BOOST.inputs['Perceptual'].default_value = PROPS.color_boost_perceptual


def update_split_tone(self, context, RR_group=None, layer_index=None):
    RR = Settings(context, RR_group, layer_index)
    PROPS = RR.props_post
    SPLIT = RR.nodes_post['Split Tone']

    if not is_layer_used(RR) or not RR.use_colors or PROPS.split_tone == 0:
        SPLIT.mute = True
    else:
        SPLIT.mute = False
        SPLIT.inputs['Factor'].default_value = PROPS.split_tone


def update_density(self, context, RR_group=None, layer_index=None):
    RR = Settings(context, RR_group, layer_index)
    DENSITY = RR.nodes_post['Density']
    PROPS = RR.props_post

    if not is_layer_used(RR) or not RR.use_colors or PROPS.density == 0:
        DENSITY.mute = True
    else:
        DENSITY.mute = False
        DENSITY.inputs['Strength'].default_value = PROPS.density
        DENSITY.inputs['Mask Highlights'].default_value = PROPS.density_depth
...
def update_white_balance(self, context, RR_group=None, layer_index=None):
    ...
    if not is_layer_used(RR) or not RR.use_colors or is_default:
        ...
        if bpy.app.version >= (5, 0, 0):
            ...
            INNER_WB.inputs[15].default_value = expand_temperature(PROPS.temperature)
            INNER_WB.inputs[17].default_value = expand_temperature(map_range(PROPS.temperature, 0, 1, 1, 0))
            INNER_WB.inputs[16].default_value = expand_tint(PROPS.tint)
            INNER_WB.inputs[18].default_value = expand_tint(map_range(PROPS.tint, 0, 1, 1, 0))
        elif bpy.app.version >= (4, 5, 0):
            WB.inputs['Temperature'].default_value = PROPS.temperature
            WB.inputs['Tint'].default_value = PROPS.tint
            WB.inputs['Perceptual'].default_value = PROPS.white_balance_perceptual
            INNER_WB = WB_NODES['White Balance']
            INNER_WB.inputs[14].default_value = expand_temperature(PROPS.temperature)
            INNER_WB.inputs[16].default_value = expand_temperature(map_range(PROPS.temperature, 0, 1, 1, 0))
            INNER_WB.inputs[15].default_value = expand_tint(PROPS.tint)
            INNER_WB.inputs[17].default_value = expand_tint(map_range(PROPS.tint, 0, 1, 1, 0))
...
def update_hue(self, context, RR_group=None, layer_index=None):
    RR = Settings(context, RR_group, layer_index)
    PROPS = RR.props_post
    HUE_PRE = RR.nodes_pre['Hue Correct']
    HUE_POST = RR.nodes_post['Hue Correct']
    ...
    if not is_layer_used(RR) or not RR.use_colors or (
        all([x == 0.5 for x in hue.values()]) and
        all([x == 1 for x in sat.values()]) and
        all([x == 1 for x in val.values()])
    ):
        HUE_PRE.mute = True
        HUE_POST.mute = True
        return

    if PROPS.hue_linear == 0:
        HUE_PRE.mute = True
        HUE_POST.mute = False
    elif PROPS.hue_linear == 1:
        HUE_PRE.mute = False
        HUE_POST.mute = True
    else:
        HUE_PRE.mute = False
    ...
    HUE_PRE.inputs['Factor'].default_value = PROPS.hue_linear
    HUE_POST.inputs['Factor'].default_value = 1 - PROPS.hue_linear

    for HUE_CORRECT in [HUE_PRE, HUE_POST]:
...
def update_value_saturation(self, context, RR_group=None, layer_index=None):
    ...
    if bpy.app.version >= (4, 5, 0):
        ...
        if not is_layer_used(RR) or not RR.use_colors:
            ...
            shadow_saturation = PROPS.shadow_saturation
            midtone_saturation = PROPS.midtone_saturation
            highlight_saturation = PROPS.highlight_saturation
...
def update_color_balance(self, context, RR_group=None, layer_index=None):
    ...
    is_default_ops = (
        ...
        get_rgb(RR.props_pre.slope_color) == [1, 1, 1]
    )
    is_default_lgg = (
    ...
    if not is_layer_used(RR) or not RR.use_colors or is_default_ops:
        OPS.mute = True
    else:
        OPS.mute = False

        if bpy.app.version >= (5, 0, 0):
            OPS.inputs[9].default_value = RR.props_pre.offset_color
            OPS.inputs[11].default_value = RR.props_pre.power_color
            OPS.inputs[13].default_value = RR.props_pre.slope_color
            OPS.inputs['Fac'].default_value = RR.props_pre.ops_factor
    ...
    if not is_layer_used(RR) or not RR.use_colors or is_default_lgg:
        LGG.mute = True
    else:
        LGG.mute = False

        if bpy.app.version >= (5, 0, 0):
            LGG.inputs[3].default_value = RR.props_post.lift_color
            LGG.inputs[5].default_value = RR.props_post.gamma_color
            LGG.inputs[7].default_value = RR.props_post.gain_color
            LGG.inputs['Fac'].default_value = RR.props_pre.lgg_factor
...
def update_color_blending(self, context, RR_group=None, layer_index=None):
    ...
    if not is_layer_used(RR) or not RR.use_colors:
        ...
        if bpy.app.version >= (4, 5, 0):
            ...
            CB.inputs['Shadow Factor'].default_value = PROPS.shadow_factor
            CB.inputs['Midtone Factor'].default_value = PROPS.midtone_factor
            CB.inputs['Highlight Factor'].default_value = PROPS.highlight_factor
            ...
            S_FAC.default_value = PROPS.shadow_factor
            M_FAC.default_value = PROPS.midtone_factor
            H_FAC.default_value = PROPS.highlight_factor
...
def update_color_panel(self, context, RR_group=None, layer_index=None):
    ...
    update_density(self, context, RR_group, layer_index)
