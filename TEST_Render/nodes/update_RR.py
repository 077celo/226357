...
from ..utilities.nodes import is_RR_node, get_groups_recursively, remove_unused_groups, clean_group_names
from ..utilities.settings import Settings
from ..utilities.presets import get_active_props_from_key, preset_settings_to_skip
from ..utilities.version import upgrade_group, is_incompatible_group
...
def update_all(self, context, RR_group=None, layer_index=None):
    print('Render Raw: Updating all properties')
...
def reset_RR(context):
    if hasattr(context.scene, 'render_raw'):
        ...
        set_curve_node(RR.nodes_pre['Curves'], RGB_curve_default)
...
@cacheless
def refresh_RR_nodes(context, include_current=True):
    NODES = scene_nodes(context)
    GROUPS = bpy.data.node_groups

    active_group_name = get_active_group(context).name

    #Format: {Group 1: [Node 1, Node 2, Node 3], Group 2: [Node 4, Node 5]}
    nodes_to_replace = {}
    ...
    default_RR = None
    nodes_to_keep = ['.RR_Pre', '.RR_Post', '.RR_color_blending']

    for group_name in nodes_to_replace.keys():
        if include_current or is_incompatible_group(GROUPS[group_name], use_cache=False):
            # Delete subgroups to avoid name conflicts
            sub_groups = [
                x for x in get_groups_recursively(GROUPS[group_name].nodes) if not any(i in x.name for i in nodes_to_keep)
            ]
            ...
            new_RR = upgrade_group(context, GROUPS[group_name], default_RR, include_current=True)
            for node in nodes_to_replace[group_name]:
                node.node_tree = new_RR

    set_active_group(context, GROUPS[active_group_name])
    remove_unused_groups()
    clean_group_names()
