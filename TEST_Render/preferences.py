...
from .handlers.toggle_handlers import toggle_handlers
...
class render_raw_preferences(bpy.types.AddonPreferences):
    ...
    enable_properties_editor: bpy.props.BoolProperty(
        name = 'Enable in Properties Editor',
        default = True
    )
    ...
    transform_during_render: bpy.props.BoolProperty(
        ...
        default = False,
        update = lambda self, ctx: setattr(self, 'requires_restart', True),
    )
    ...
    custom_transforms: bpy.props.BoolProperty(
        name = 'Allow Custom Color Transforms',
        description = ('Allows Render Raw to detect color transforms from custom OCIO setups such as Pixel Manager'),
        default = True
    )
    requires_restart: bpy.props.BoolProperty(
        default = False
    )
    ...
    def draw(self, context):
        ...
        # interface.prop(self, 'enable_properties_editor', text='Properties Editor')
        ...
        col.row(heading='Enable').prop(self, 'transform_during_render', text='Transform During Render (Unstable)')
        col.prop(self, 'animated_values', text='Animated Values (Unstable)')
        if self.requires_restart:
            split = col.split(factor=0.4)
            col1 = split.column(align=True)
            col1.alignment = 'RIGHT'
            col2 = split.column(align=True)
            col2.label(text='Save preferences and restart to apply changes', icon='ERROR')
        col.separator()

        col.separator()
        col.label(text="Customizing Blender")
        row = col.row()
        row.label(text=" ")
        row.operator('wm.url_open', text="Get More Color Transforms").url = "https://github.com/Joegenco/PixelManager"
        row.operator('wm.url_open', text="Installing Color Transforms").url = "https://cgcookie.github.io/render_raw/08_customizing_blender.html"
