...
def draw_color_management(self, context, RR):
    ...
    if (
        ...
        (RR.props_scene.enable_RR and RR.is_incompatible)
        or (not RR.props_scene.enable_RR and hasattr(context.scene, 'render_raw') and context.scene.render_raw.enable_RR)
    ):
        draw_upgrade_panel(self, col, context, RR)
        return

    row = col.row(heading='Render Raw')
    row.prop(RR.props_scene, 'enable_RR', text='Enable')
    col.separator()
    ...
    if (
        ...
        row = col.row()
        row.prop(RR.props_group, 'view_transform', text='Transform')
        row.popover('RENDER_PT_render_raw_working_space', icon='OPTIONS', text='')
...
def draw_curve_option(layout, nodes, props, position):
    layout.use_property_split = False
    layout.row().prop(props, f'{position}_curves_tone', text='')
    layout.separator()
    layout.template_curve_mapping(
        nodes, "mapping", type='COLOR', levels=False, show_tone=False
    )
    row = layout.row(align=True)
    row.prop(props, f'{position}_curves_black', text='')
    row.prop(props, f'{position}_curves_white', text='')
    layout.separator()
    layout.prop(props, f'{position}_curves_factor', text='Factor', slider=True)
...
def draw_curves(context, layout, RR):
    ...
    if panel:
        ...
        if RR.props_group.display_curves == 'PRE':
            draw_curve_option(layout, RR.nodes_pre['Curves'], RR.props_pre, 'pre')
        elif RR.props_group.display_curves == 'LOG':
            draw_curve_option(layout, RR.nodes_pre['Log Curves'], RR.props_pre, 'log')
        elif RR.props_group.display_curves == 'POST':
            draw_curve_option(layout, RR.nodes_post['Curves'], RR.props_post, 'post')
...
def draw_details(layout, props_pre, props_post):
    ...
    if panel:
        ...
        if bpy.app.version >= (4, 5, 0):
            col.separator()
            col.prop(props_post, 'negative_bleed', text='Shadow Bleed', slider=True)
            col.prop(props_post, 'negative_bleed_size', slider=True)
...
def draw_values(self, context, RR):
    ...
    if get_prefs(context).enable_layers:
        ...
        col.separator()
    col.prop(RR.props_pre, 'contrast', slider=True, text='Contrast Total')
    col.prop(RR.props_pre, 'contrast_shadows', slider=True, text='Shadows')
    col.prop(RR.props_pre, 'contrast_highlights', slider=True, text='Highlights')
    col.separator()
    col.prop(RR.props_post, 'blacks', slider=True, text='Levels Blacks')
    col.prop(RR.props_post, 'shadows', slider=True)
    col.prop(RR.props_post, 'highlights', slider=True)
    col.prop(RR.props_post, 'whites', slider=True)
    col.separator()
...
def draw_per_hue(layout, props_node, props_pre, props_post):
    ...
    if panel:
        ...
        col.prop(props_post, 'hue_linear', text='Linear Blend', slider=True)
        if bpy.app.version >= (4, 5, 0):
...
def draw_colors(self, RR):
    ...
    col.separator()
    col.prop(RR.props_post, 'split_tone', slider=True)
    col.separator()
    ...
    col.separator()
    col.prop(RR.props_post, 'density', slider=True)
    col.prop(RR.props_post, 'density_depth', text='Value Mask', slider=True)
    col.separator()
...
def draw_glare(layout, props, is_enabled):
    ...
    if not panel:
        ...
        if bpy.app.version >= (4, 5, 0):
            glare.prop(props, 'halation', slider=True)
            glare.prop(props, 'halation_size', slider=True, text='Size')
            glare.separator()
...
def draw_vignette(layout, props, is_enabled):
    ...
    if not panel:
        ...
        vig.prop(props, 'vignette_linear_blend', text='Linear Blend', slider=True)
...
def draw_effects(self, RR):
    ...
    if bpy.app.version >= (4, 5, 0):
        col.prop(RR.props_post, 'horizontal_dispersion', slider=True)
...
def draw_alpha_panel(layout, context, props):
    header, panel = layout.panel(idname='RR_alpha_panel', default_closed=False)
    header.label(text="Alpha")
    if panel:
        panel.row(heading='Film').prop(context.scene.render, 'film_transparent')
...
def draw_utilities(self, context, RR):
    ...
    draw_alpha_panel(layout, context, RR.props_group)

    if RR.props_group.view_transform == 'sRGB' and bpy.app.version < (5, 0, 0):
    ...
    if panel:
        panel.prop(context.scene.display_settings, 'display_device')
